...
name: canvas
description: >-
  Build and edit canvases, single `.canvas.tsx` files the IDE compiles so the
  user can open the result beside the chat. Use when the deliverable is
  standalone analytical output (quantitative analyses, billing or account
  investigations, security audits, architecture reviews, data-heavy reports,
  charts, tables, interactive explorations) or structured data from MCP tools,
  and whenever creating, editing, or debugging a `.canvas.tsx` file. Skip when
  the user wants work in a specific external tool or changes to an existing
  artifact that is not a canvas.
metadata:
  surfaces:
    - ide
    - cloud
